...
export default function PaymentsPage() {
  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/payments/?status=${filter}&range=${dateRange}`, {
        headers: {
  ...
  const handlePaymentAction = async (paymentId, action) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/payments/${paymentId}/${action}/`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${session.accessToken}`,
        },
                ...
                <div key={payment.id} className="p-6 hover:bg-gray-50">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-500 mb-1">
                        Landlord: {payment.listing?.landlord?.name || 'N/A'}
                      </p>
                      <p className="text-sm text-gray-500 mb-1">
